...
const CORE_APPS: IconLinkItem[] = [
  ...
  { label: "eLRMS", icon: FileText, badge: "x2" },
  { label: "MIS Hydro", icon: Droplet },
];

const OPERATIONS: IconLinkItem[] = [
  { label: "Wrench", icon: Wrench, badge: "x3" },
  { label: "E log", icon: FileOutput, badge: "x3" },
  { label: "MDG", icon: Database, badge: "x2" },
    ...
    {items.map(({ label, icon: Icon, badge }) => (
      <li key={label}>
        ...
        >
          ...
          <span>{label}</span>
          {badge && (
            <span className="ml-auto text-[11px] font-medium text-slate-500 bg-slate-100 rounded-md px-1.5 py-0.5">
              {badge}
            </span>
          )}
        </a>
